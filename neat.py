...
def genRandNet(numInputs, numOutputs, layerRange, neuronRange, weightRange, connectionRange):
    ...
    for x in range(numLayers):
        ...
        for y in range(numNeurons):
            if(x == 0):
                neurons.append(Neuron(randint(weightRange[0],weightRange[1]),input=True))
            else:
                neurons.append(Neuron(randint(weightRange[0],weightRange[1])))

        layers.append(Layer(neurons=neurons))
    network.layers = layers


    #setup input layer to randint
    for n in network.layers[0].neurons:
        n.connections = [ Connection(Input(),randint(weightRange[0], weightRange[1])) ]
    ...
    for x in range(1,numLayers):
        ...
        for n in childLayer.neurons:
            ...
            if(numConnections > numParentNeurons):
                numConnections = randint(1, numParentNeurons)


            usedParents = []
            for i in range(numConnections):
                rWeight = randint(weightRange[0], weightRange[1])
